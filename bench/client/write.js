...
function doWrite (){
	...
	var start = Date.now();
	client.append(queue, function (err, results){
		var end = Date.now();
		console.log("[%s] Write %s topics(%s), %s records. err:%s, cost:%s ms.", 
			moment().format("YYYY-MM-DD HH:mm:ss"), 
			topics.length, 
			metric.name,
			topics.length * Object.keys(data).length,
			err ? (err.message ? err.message : err.code) : null,
			(end - start));	
	});
}
...
function run (){
	Q.fcall(function (){
		return Q.Promise(function (resolve, reject){
			client.getMetric("metric_bench_2", function (err, result){
				if (err)
					reject(err);			
				else{
					metric = result;
