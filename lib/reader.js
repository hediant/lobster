	...
	var readOneHandle_ = function (handle, select, metric, where){
		return Q.Promise(function (resolve, reject){
			...
			*/
			...
			var csvstream = csv.fromStream(fs.createReadStream(handle), {
				"headers":true, 
				"delimiter":config.separator
			});

			csvstream.transform(function (data){
				return transform_(handle, data, select, metric, where);
			})
			.on('data', function (data){
				rows.push(data);
			})
			.on('end', function (){
				...
				var e2 = Date.now();
				if (config.debug){
					console.log("Load [%s], cost:%s ms, sort:%s ms.", handle, (e1 - start), (e2 - e1));
				}
