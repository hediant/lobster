...
function Reader(topic_name, field_names){
	...
	*/
	...
	var Cursor = function (start, options){
		...
		var readNextEvent_ = function (buffer, handle){
			try {
				var evt = LBF.Event.nextOne(buffer);
				return evt;
			}
			catch (err){
				console.error("File %s is corrupted at #%s, err: %s, try next empty event.",
					handle,
					buffer.offset,
					err);

				return LBF.Event.nextEmptyOne(buffer);
			}
		}

		var transform_ = function (buffer, handle){
			var rows = [];
			var evt = readNextEvent_(buffer, handle);

			while(evt){
				var ts = evt.header.timestamp;
				if (!inRange_(ts) || !evt.chunk){
					// next event
					evt = readNextEvent_(buffer, handle);					
					continue;
				}
