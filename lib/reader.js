...
function Reader(topic_name, field_names){
	...
	var toMilliSeconds_ = function (ts){
		var hours = parseInt(ts.substr(0, 2));
		var minutes = parseInt(ts.substr(2, 2));
		var seconds = parseInt(ts.substr(4, 2));

		return (hours * 3600 + minutes * 60 + seconds) * 1000;
	}

	var type_cast_ = function (data, metric, ts){
		var ret = {
			"__ts__" : ts + toMilliSeconds_(data.__ts__)
		};

		metric.keys.forEach(function (field){
			var val = data[field.name];
			if (val != undefined){
				ret[field.name] = type_cast(val, field.type);
			}
		});
	...
	var transform_ = function (handle, data, select, metric, where){
		var day = getDayFromHandle(handle);
		var ts = moment(day).valueOf();

		var cast_data = type_cast_(data, metric, ts);
		var ret = {
			"__ts__" : cast_data.__ts__
		};
		...
		return where(cast_data) ? ret : null;
	}

	// @handle - string, file path
	// @select - fields of Metric, array of object
	// @metric - Metric object
	// @where - function (row)
	var readOneHandle_ = function (handle, select, metric, where){
		return Q.Promise(function (resolve, reject){
			var start = Date.now();
			var rows = [];

			fs.createReadStream(handle)
			.pipe(csv({
				"separator" : config.separator
			}))
			.on('data', function (data){
				var t = transform_(handle, data, select, metric, where);
				if (!t){
					rows.push(t);
				}
			})
			.on('end', function (){
				var end = Date.now();
				if (config.debug){
					console.log("Load [%s], cost:%s ms.", handle, (end - start));
				}
				resolve(rows);
			})
	...
	var readHandles_ = function (handles, select, metric, where, cb){
		var callbacks = [];
		handles.forEach(function (handle){
			callbacks.push(readOneHandle_(handle, select, metric, where));
		});
		...
		Q.allSettled(callbacks).then(function (results){
			...
			cb && cb(last_error, last_error ? null : rows);
		});
	...
	*/
	this.readRaw = function (){
		...
		if (options && options.limit){
			options_.limit = parseInt(options.limit) < 10000 ? parseInt(options.limit) : 10000;
		}
	...
	var Cursor = function (start, options){
		...
		var forward_ = function (cb){
			...
			if (!buffer_){
				...
				// load buffer_
				readHandles_(handles_[current_], select_fields_, metric_, options_.where, function (err, results){
					if (err){
						cb && cb(err);
					}
					else{
