	...
	var findAndEncape_ = function (h_start, h_end, selection, options){
		return Q.Promise((resolve, reject) => {
			...
			co(function *(){
				...
				var col = db.collection(region_idx_.toString());
				...
				Object.keys(selection).forEach((field_name) => {
					select_mask["fields." + field_name] = 1;
				});
	...
	var encapeResults_ = function (results, selection, options){
		...
		results.forEach((row) => {
			...
			for (var field_name in fields){
				...
				var tag = fields[field_name];
				if (!tag)
					continue;

				var data_type = selection[field_name].type;
