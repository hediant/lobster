...
function Topic (topic_name){
	...
	var region_path_ = Topic.RegionPath(topic_name);
	...
	var createWriteStream_ = function (log_file, flag, cb){
		...
		wstream.once('open', function (){
			...
			if (flag){
				...
				//
				//	If it's not the first time to create the File,
				//	and is the first to load the topic's file stream(appending),
				//	we need to write an empty event at first.
				//
				//	The purpose to do so is that
				//	if the server crashed last time, and file is damaged,
				//	we can read the events after this Empty Event.
				//
				//	thus, we can avoid data lost after last server crash.
				//
				wstream.write(LBF.Event.encapeEmptyEvent());

				// clear first load status
				first_load_ = false;
			}
	...
	var initRegionPath = function (){
		return Q.Promise(function (resolve, reject){
			fs.exists(region_path_, function (exists){
				if (exists){
					resolve();
				}
				else{
					fs.mkdir(region_path_, function (err){
						if (err)
							err == "EEXIST" ? resolve() : reject(err);
						else
							resolve();
...
Topic.RegionPath = function (topic_name){
	return path.join(config.db_path, regionOfTopic(topic_name).toString());
}

Topic.BasePath = function (topic_name){
	return path.join(Topic.RegionPath(topic_name), topic_name);
}
